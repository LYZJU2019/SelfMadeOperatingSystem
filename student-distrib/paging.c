...
void init_page_dir()
{
    // ! We don't have B8000 in graphical mode!
    // init the first entry (4KB indirection, user space, containing video memory).
    {
        ...
        pde->reserved = 0;
        pde->pg_addr = 0;       // Bits 31-12 of address.
...

    {
		...
		pde->dirty = 1; // Has been written.
		pde->pageSize = 1; // 4MB page for kernel space
		pde->global = 1; // Global page ignore.
		pde->avail = 0; // Not used.
		pde->pg_attri = 0; // Reserved and be set to 0.
		pde->addr_0 = 0; // Bits 39-32 of address.
		pde->reserved = 0;
		pde->pg_addr = (uint32_t) vbe_idx; // Bits 31-12 of address.
		page_dir_base[vbe_idx] = the_page_dir_entry;
	}

    {
		int idx = TERM_1_4M_VIDEO >> 22;
		page_dir_entry_u the_page_dir_entry;
		page_dir_entry_4MB_t* pde = &(the_page_dir_entry.kernel_page_desc);
		pde->present = 1; // In memory.
		pde->r_w = 1;
		...
		pde->addr_0 = 0; // Bits 39-32 of address.
		pde->reserved = 0;
		pde->pg_addr = (uint32_t) idx; // Bits 31-12 of address.
		page_dir_base[idx] = the_page_dir_entry;
	}

    {
		int idx = TERM_2_4M_VIDEO >> 22;
		page_dir_entry_u the_page_dir_entry;
		page_dir_entry_4MB_t* pde = &(the_page_dir_entry.kernel_page_desc);
		pde->present = 1; // In memory.
		pde->r_w = 1;
    ...
    int i;
    for (i = 0; i < NUM_PAGE_TABLE_DESC; i++)
    {
        if ((i == 0) || (i == 1) || (i == VBE_START >> 22) || (i == TERM_1_4M_VIDEO >> 22) || (i == TERM_2_4M_VIDEO >> 22) || (i == TERM_3_4M_VIDEO >> 22) || (i == ICON_START >> 22) || i == 124 / 4)
            continue;
        page_dir_entry_u the_page_dir_entry;
        page_dir_entry_4MB_t *pde = &(the_page_dir_entry.kernel_page_desc);
...
void switch_usrmap(int index, int present)
{
    ...
    pde->dirty = 1; // Has been written.
    pde->pageSize = 1; // 4MB page for kernel space
    pde->global = present; // Global page ignore.
    pde->avail = 0; // Not used.
    pde->pg_attri = 0; // Reserved and be set to 0.
    pde->addr_0 = 0; // Bits 39-32 of address.
    pde->reserved = 0;
    pde->pg_addr = (uint32_t) index; // Bits 31-12 of address.
    page_dir_base[FISH_VIRTUAL >> 22] = the_page_dir_entry;
}
