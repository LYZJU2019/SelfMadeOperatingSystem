...
void pit_int_handler(void)
{
    cli();
    send_eoi(PIT_IRQ);
    // switch paging before call show screen
    // change_vidmem_mapping(viewing_term_ptr->tid);
    // VGA_showscreen();
    // restore paging

    // change_vidmem_mapping(running_term_ptr->next_term_to_run->tid);
    screen_x_ptr = &term_arr[running_term_ptr->next_term_to_run->tid].cursor_x;
    screen_y_ptr = &term_arr[running_term_ptr->next_term_to_run->tid].cursor_y;
