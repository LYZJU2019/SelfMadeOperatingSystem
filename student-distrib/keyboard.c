    ...
    else
    {
        ...
        if (0x02 <= input_code && input_code <= SCAN_CODE_SET_SIZE - 1)
        {
            ...
            else
            {
                ...
                screen_x_ptr = &term_arr[viewing_term_ptr->tid].cursor_x;
                screen_y_ptr = &term_arr[viewing_term_ptr->tid].cursor_y;
                // switchpagingtovideomem(0);
                kbd_putc(what_to_put);

                // calib_cursor();
                screen_x_ptr = &term_arr[running_term_ptr->tid].cursor_x;
                screen_y_ptr = &term_arr[running_term_ptr->tid].cursor_y;
